defmodule Nerves.Runtime.HeartTest do
  ...
  describe "parse_cmd/1" do
    test "Raspberry Pi w/ v1 hex options" do
      cmd =
        ~c"program_name=nerves_heart\nprogram_version=1.0.0\nidentity=Broadcom BCM2835 Watchdog timer\n" ++
          ~c"firmware_version=0\noptions=0x00008180\ntime_left=13\npre_timeout=0\ntimeout=15\nlast_boot=power_on\n"
    ...
    test "BBB w/ v1 options" do
      cmd =
        ~c"program_name=nerves_heart\nprogram_version=1.0.0\nidentity=OMAP Watchdog\nfirmware_version=0\n" ++
          ~c"options=settimeout,magicclose,keepaliveping,\ntime_left=119\npre_timeout=0\n" ++
          ~c"timeout=120\nlast_boot=power_on\n"
    ...
    test "RaspberryPi 3 w/ v2.2 options" do
      cmd =
        ~c"program_name=nerves_heart\nprogram_version=2.2.0\nheartbeat_timeout=30\nheartbeat_time_left=29\n" ++
          ~c"init_grace_time_left=0\nsnooze_time_left=0\nwdt_pet_time_left=6\ninit_handshake_happened=1\n" ++
          ~c"init_handshake_timeout=0\ninit_handshake_time_left=0\nwdt_identity=Broadcom BCM2835 Watchdog timer\n" ++
          ~c"wdt_firmware_version=0\nwdt_options=settimeout,magicclose,keepaliveping,\nwdt_time_left=14\n" ++
          ~c"wdt_pre_timeout=0\nwdt_timeout=15\nwdt_last_boot=power_on\n"
    ...
    test "No options" do
      cmd = ~c"program_name=nerves_heart\nprogram_version=1.0.0\noptions=\n"
    ...
    test "Erlang heart" do
      assert {:error, :not_nerves_heart} = Heart.parse_cmd(~c"")
      assert {:error, :parse_error} = Heart.parse_cmd(~c"reboot")
    end

    test "parse errors" do
      assert {:error, :parse_error} = Heart.parse_cmd(~c"program_version=1.0")
      assert {:error, :parse_error} = Heart.parse_cmd(~c"reboot")
    end
  end
end
